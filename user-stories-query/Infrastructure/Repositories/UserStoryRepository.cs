...
public class UserStoryRepository : IUserStoryRepository
{
    ...
    public async Task<IEnumerable<IUserStory>> GetAllAsync()
    {
        var sprintDataModels = await _context.Set<UserStoryDataModel>().ToListAsync();
        var result = sprintDataModels.Select(_mapper.Map<UserStoryDataModel, UserStory>);
